...
import { archiveGroup, updateGroup } from "@/lib/data";
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const group = await updateGroup(id, parseGroupPayload(body));
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await context.params;
    const group = await archiveGroup(id);

    return NextResponse.json({ group });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to archive group." }, { status: 400 });
  }
}
