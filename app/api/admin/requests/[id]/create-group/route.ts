...
import { createGroupFromRequest } from "@/lib/data";
...
function parsePayload(body: Record<string, unknown>) {
  ...
  if (!name || !inviteLink || !categoryId || !description) {
    throw new Error("Group name, invite link, category and description are required.");
  }
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await createGroupFromRequest(id, parsePayload(body));

    return NextResponse.json(result, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to create group from request." }, { status: 400 });
  }
}
